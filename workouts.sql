with pedometer_intervals as (
    select * from analytics.dbt_claire.pedometer_intervals
),

jump_sections as (
    select * from analytics.dbt_claire.jump_sections
),

break_sections as (
    select * from analytics.dbt_claire.break_sections
),
...
workout_streaks as (
    select
        ...
        count(*) as n_streaks,
        avg(total_steps) as average_steps_per_workout,
        median(total_steps) as median_steps_per_workout,
